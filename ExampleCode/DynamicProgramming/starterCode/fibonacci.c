...
u_int64_t fibonacci_recursive(int n) {
  if (n <= 1){
    return n;
  }
...
u_int64_t fibonacci_memoization(int n, u_int64_t *memo) {
  if (n <= 1){
    memo[n] = n;
  }
  if (memo[n] != 0 || n == 0){
...
u_int64_t time_fibonacci_recursive(int n) {
  clock_t start = clock();
  u_int64_t result = fibonacci_recursive(n);
  clock_t end = clock();
  printf("Fibonacci(%d) = %ld\n", n, result);
  return (int)(end - start);
}

u_int64_t time_fibonacci_memoization(int n) {
  u_int64_t *memo = (u_int64_t *)malloc((n + 1) * sizeof(u_int64_t));
  memset(memo, 0, (n + 1) * sizeof(int)); // Initialize memo array to 0
  clock_t start = clock();
  u_int64_t result = fibonacci_memoization(n, memo);
  clock_t end = clock();
  printf("Fibonacci(%d) = %ld\n", n, result);
